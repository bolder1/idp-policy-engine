...
describe('reading who out of a predicate', () => {
  it('finds the ids a rule names, and nothing when it names none', () => {
    const p = when(card(cond('group', 'in', ['finance', 'eng']), cond('country', 'is', ['India'])))
  ...
  it('separates who from circumstance, so the two steps cannot show the same row', () => {
    const p = when(card(cond('group', 'in', ['finance']), cond('user', 'is', ['priya']), cond('country', 'is', ['India'])))
    expect(restConditions(p).map((c) => c.typeId)).toEqual(['country'])
  })
})

describe('writing who back', () => {
  it('adds one condition for a kind that had none', () => {
    const p = setWho(when(card(cond('country', 'is', ['India']))), 'group', ['finance'])
    expect(leaves(p).map((c) => `${c.typeId} ${c.operator} ${c.values}`)).toEqual(['country is India', 'group in finance'])
  })
  ...
  it('never grows a second condition for the same kind', () => {
    let p = setWho(when(card(cond('country', 'is', ['India']))), 'group', ['finance'])
  ...
  it('keeps the operator so a narrowing can be an exclusion', () => {
    let p = setWho(when(card(cond('country', 'is', ['India']))), 'group', ['contractors'])
...
describe('when the step refuses to edit', () => {
  ...
  it('declines a predicate with two alternatives', () => {
    expect(whoEditable(when(card(cond('country', 'is', ['India'])), card(cond('city', 'is', ['Pune']))))).toBe(false)
  })

  it('declines a single card whose conditions are alternatives', () => {
    expect(whoEditable({ cards: [{ ...card(cond('country', 'is', ['India'])), join: 'or' }] })).toBe(false)
  })

  it('accepts an AND-run and the catch-all', () => {
    expect(whoEditable(when(card(cond('country', 'is', ['India']))))).toBe(true)
